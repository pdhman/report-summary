...
5. **페이지네이션 종료 조건** — 각 페이지에서 **최상위(정렬 위치) 트윗의 최소 created_at**
   ...
   **대화 모듈(`content.items[]`)의 부모 트윗은 종료 조건 계산에서 빼라** — 모듈엔 몇 시간~며칠 전
   원글이 섞여 있어 최솟값을 끌어내린다. 단일 엔트리(`content.itemContent`)와 모듈의 **마지막
   항목**만 쓴다. (2026-09-11 실제 사고: 3페이지에 섞인 전날 21:40Z 부모 글 때문에 조기 종료 →
   09-10 09:05~15:28 KST 12건이 누락됐다. 이 조건을 고친 뒤 4페이지까지 가서 전부 회수.)
...
> **중복 실행 주의 (2026-08-31 실제 사고).** `last_run`은 실행이 **끝날 때** 기록되므로,
> 앞선 실행이 아직 돌고 있는 중에 다음 실행이 시작되면 두 실행이 겹친다. 게다가 앞선
> 실행이 자기 수집 시작 시각을 `last_run`으로 쓰면서 **그 이전 구간이 영구히 유실**될 수
> 있다(당시 금요일 오전 KST 9시간이 통째로 빠졌고, 같은 날짜 파일 안에서도 38건이 더
> 누락됐다). 실행 시작 시 **`data/오늘.json`이 이미 있으면 그 파일의 min/max `time`을 먼저
> 확인**하고, 직전 날짜 파일의 max `time`과 이어지는지 검증할 것. 끊겨 있으면 그 구간까지
> 덮도록 더 거슬러 올라가 수집하고, url 기준으로 병합한다(같은 날짜 파일 병합은 원래 절차).
> 리포트도 병합 후 전체 구간 기준으로 다시 쓴다. 단, **텔레그램이 이미 발송된 날이면
> (`telegram/sent.json`에 오늘 날짜가 있으면) 재발송하지 않는다** — 공개 채널 중복 게시가 된다.
...
- **길이**: 약 1,000자(escape 포함 문자열 기준)에서 `[TRUNCATED]`로 잘린다 (요청한 slice 길이와 무관).
  → 청크는 개수가 아니라 **`JSON.stringify` 길이 800자 기준으로 그리디 패킹**하라. 1,050자로 묶었을 땐
  119청크 중 10개가 잘렸고, 800자로는 78청크 중 0개였다(2026-09-14, 301건).
  (배치 도중 다른 Chrome 확장이 재연결되면 **세션의 브라우저 선택이 풀려** 배치 전체가 에러난다 —
  같은 deviceId로 `select_browser`만 다시 하면 탭·전역변수는 그대로 살아 있다.)
...
**`window.name` 우회도 불가** (2026-08-31 확인) — `window.name`은 크로스오리진 내비게이션에서
살아남으므로 "x.com에서 window.name에 담고 → 같은 탭을 로컬 페이지로 이동 → 그 페이지가
동일 오리진으로 POST" 경로가 이론상 성립한다. 실제로 CSP는 이 경로를 막지 못한다.
그러나 **확장의 도메인 정책이 `navigate`를 거부한다**(`Navigation to this domain is not allowed`,
`about:blank`도 `Invalid URL`로 거부). 즉 CSP가 아니라 확장 쪽에서 막히므로 시도할 필요 없다.
