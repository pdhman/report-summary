...
작업 스케줄러에 **`크립토모니터_1100_수집`**·**`크립토모니터_2000_수집`** 두 개로 등록됨
(매일 11:00·20:00, 둘 다 `run_crypto.ps1`).

- **11시**: UTC 일봉이 09:00 KST에 마감되므로 그 뒤라야 전일 UTC 데이터(MVRV·OI·가격)가 완성된다.
- **20시**: ETF 순유입 때문. Farside는 발행사 보고가 들어오는 대로 칸을 채우고 영국 오전에
  마무리하므로, 11시 KST(미국 전날 22시)엔 전일 세션이 대개 **부분집계**다(2026-09 기준 9번 중
  5번, 9/2는 −14.3 → 최종 +101.1로 부호까지 바뀜). 파서가 부분집계 행을 거르므로 11시 실행에선
  ETF 기준일이 T-2로 보이고, 20시(영국 12시) 실행이 T-1을 채운다. 로그의 "미집계 행 건너뜀:
  날짜 (n/m 종목 보고)"로 그 시각의 집계 진행률을 볼 수 있으니, 20시에도 자주 걸리면 더 늦춘다.
...
Farside는 아직 집계되지 않은 당일과 미국 증시 휴장일도 표에 행으로 게시한다. 휴장일·미집계는
개별 종목 칸이 전부 `-`인 채 **Total만 `0.0`**(2026-08-31 발견 당시 17일 오염), 집계 중인 날은
**먼저 보고한 발행사 칸만 숫자**라 Total이 부분합이다(2026-09-14 11시: 12종목 중 1개, Total 9.7).
`_parse_farside_page()`는 종목 칸의 **3/4 이상이 숫자이고 첫 칸(블랙록)도 숫자인 행만** 인정한다 —
전체 이력 1,207행이 모두 이 기준을 넘어 정상일은 하나도 안 걸린다. 표가 보여주는 날짜 범위 안에서
표가 인정하지 않는 날은 캐시에서도 지우므로(자가 치유), 부분집계가 캐시에 남았다가 굳는 일이 없다.
