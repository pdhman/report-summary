  ...
  <section id="sec-supply">
    ...
    <div class="cards">
      ...
      <div class="card"><h3>데이터센터 공실률 (%)</h3>
        <p class="note" id="vacNote">CBRE 반기 보고서 → manual_data.json 수동 입력</p>
...
function renderAll(){
  ...
  if(vac.length){
    ...
    const vl=vac[vac.length-1], vp=vac.length>1?vac[vac.length-2]:null;
    document.getElementById('vacNote').textContent=
      `CBRE 반기 보고서(${vl.region||''}, 수동 입력) — 최신 ${vl.date} ${fmt(vl.vacancy_pct)}%`+
      (vp?` · 직전 ${vp.date} ${fmt(vp.vacancy_pct)}%`:'')+' · 반기마다 발표(다음: 하반기분, 이듬해 2~3월)';
  } else {
