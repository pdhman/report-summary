...
| ② 수요둔화 | 2022 긴축 | Hyperscaler 매출 YoY, 클라우드 부문 YoY, NVDA DC 매출 YoY, 5사 연간 capex(컨센서스 vs 실적·런레이트), MU 매출 YoY(HBM 프록시), AI 토큰 P×Q(제번스 모니터) | Yahoo·SEC·OpenRouter(자동) · 실적발표·Bloomberg(수동) |
...
>
> **AI 토큰 P×Q (제번스 모니터, 2026-09 추가)**: UBS "토큰 단가 절반↓ vs DRAM 가격↑" 논지의
> 정량화. 총지출 = 단가(P) × 처리량(Q) — 단가가 떨어져도 처리량이 더 빨리 늘면 총 컴퓨팅 수요는
> 증가(제번스 역설), 지출이 꺾이면 효율이 수요를 이기는 전환. 소스는 OpenRouter 무인증 API
> (`/api/v1/models` 정가 + `/api/frontend/v1/rankings/models?view=week` 모델별 주간 토큰):
> P = Σ(모델별 토큰×정가)/Σ토큰(사용량 가중, Silicon Data SDLLMTK 지수와 같은 방식이나 정가·
> 개발자 표본 기준이라 절대 레벨은 다름 — 2026-09 첫 주 $0.56 vs SDLLMTK $0.99), Q = 총토큰/주.
> 히스토리는 Wayback의 API 아카이브(2026-06~)로 백필, 이후 매주 `token_pxq_history.csv` 누적.
> 경계: P×Q 13주 변화 0% 이하.
>
> **Compute/Task 프록시 (2026-09-08 추가)**: "토큰 수 → 작업당 계산량"으로 측정 단위가 바뀐다는
> 논지(recurrent depth·에이전트)의 정량화. 같은 OpenRouter 응답에서 요청 수(작업 수), 요청당
> 토큰(작업 무게), 프론티어 티어 단가(사용량 상위 100개 중 실효단가 상위 5개의 가중 단가 —
> 랩이 매긴 compute 원가의 그림자), 티어 격차, 중국계 모델 점유율을 산출해 `compute_task_history.csv`
> 누적. 추론 토큰·툴 호출 필드는 OpenRouter 집계값이 0이라 사용 불가. **토큰에 잡히지 않는
> 내부 계산은 정의상 토큰 데이터로 못 보므로**, 하드웨어 측 확인용 GPU 가동률 프록시
> (500.farm 임대중÷(임대중+가용), H100·B200)를 같이 수집 — 공급 축 경계 60% 미만(유휴 증가),
> 수요 축 경계 요청 수 13주 0% 이하(작업 수 정체).
...
> 캐시가 있으면 마지막 날짜 30일 전부터만(`cosd=`) 받아 병합 — 프록시가 전체 히스토리 응답에서 520/522를 자주 내기 때문.
> 클라우드 부문 판정은 '각사 최신 분기' 기준(오라클만 먼저 발표된 부분 분기에 OCI 하나로 판정하지 않음).
