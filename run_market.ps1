...
if ($branch -ne 'main') {
    ...
    if ((git rev-parse --abbrev-ref HEAD 2>$null) -ne 'main') {
        "$((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')) [market] branch '$branch' -> main FAILED (uncommitted edits in the way?) - abort, nothing discarded" |
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        git commit -m ("market: {0:yyyy-MM-dd} 시장 온도계 갱신" -f (Get-Date)) 2>&1 | Add-Content -Path $log -Encoding UTF8
